...
function performWork(deadline) {
  ...
  if (nextUnitOfWork || updateQueue.length > 0) {// 本次更新存在未执行完工作或者队列存在其他更新任务
    requestIdleCallback(performWork);// 任务放入下次空闲时间执行
  }
}
// 执行Xeact核心对比工作
function workLoop(deadline) {
  if (!nextUnitOfWork) {// 本次更新无任务执行，则取出更新队列中的任务
    resetNextUnitOfWork();
  }
  while (nextUnitOfWork && deadline.timeRemaining() > ENOUGH_TIME) {// 存在未完成工作和剩余时间
    nextUnitOfWork = performUnitOfWork(nextUnitOfWork);// 循环执行
  }
  if (pendingCommit) { // 本次工作全部完成，提交全部dom变更
...
function resetNextUnitOfWork() {
  ...
  const root =
    update.from == HOST_ROOT
      ...
      : (update.from == FUNCTION_COMPONENT ? getRoot(update.alternate) :
...
function performUnitOfWork(wipFiber) {
  ...
  if (wipFiber.child) {// 存在第一个子元素则返回子元素，继续执行工作（深度优先遍历）
  ...
  while (uow) {
    ...
    if (uow.sibling) {// 找到后面的兄弟元素，继续执行工作
...
function updateClassComponent(wipFiber) {
  ...
  if (instance == null) {//组件第一次创建，实例为空，创建一个实例并赋值给stateNode
...
function reconcileChildrenArray(wipFiber, newChildElements) {
  ...
  while (index < elements.length || oldFiber != null) {
    ...
    if (sameType) {//类型相同 newFiber打上更新tag
      newFiber = {
        type: oldFiber.type,
        tag: oldFiber.tag,
    ...
    if (element && !sameType) {//类型不同，存在新元素 newFiber打上新增tag
      newFiber = {
        type: element.type,
        tag:
          typeof element.type === "string" ? HOST_COMPONENT : (typeof element.type === 'function'? FUNCTION_COMPONENT:CLASS_COMPONENT),
        props: element.props,
        parent: wipFiber,
        effectTag: PLACEMENT
      };
    ...
    if (oldFiber && !sameType) {// 类型不同，存在旧元素 oldFiber打上删除tag
      oldFiber.effectTag = DELETION;
      wipFiber.effects = wipFiber.effects || [];
      wipFiber.effects.push(oldFiber);// 删除不会创建newFiber，所以直接将oldFiber加入到父元素的effects
    }

    if (oldFiber) {// 遍历fiber的兄弟元素
      oldFiber = oldFiber.sibling;
    }

    if (index == 0) {// 把第一个元素引用放在父元素的child中
...
function commitAllWork(fiber) {
  fiber.effects.forEach(f => {// 循环提交工作
    commitWork(f);
  });
  // 重置
...
function commitWork(fiber) {
  if (fiber.tag == HOST_ROOT) {// 如果是根fiber则不执行
    return;
  }
  ...
  if (fiber.effectTag == PLACEMENT && fiber.tag == HOST_COMPONENT) {// 新增
...
function commitDeletion(fiber, domParent) {
  let node = fiber;
  while (true) {
    if (node.tag == CLASS_COMPONENT) {// 找到最近的一个tag不为虚拟dom的子级fiberA
      node = node.child;
      continue;
    }
    domParent.removeChild(node.stateNode);// 直接移除该元素
    while (node != fiber && !node.sibling) {// 如果fiberA不是fiber，并且不存在兄弟fiber，则继续查找fiberA父级
      node = node.parent;
    }
    if (node == fiber) {// 直到fiberA回到最初的fiber。循环结束，dom删除完毕
      return;
    }
...
function useState(initial){
  ...
  const oldHook = wipFiberRoot.alternate && wipFiberRoot.alternate.hooks &&
  wipFiberRoot.alternate.hooks[hookIndex];
  const hook = {state: oldHook? oldHook.state: initial,queue:[]};
  const actions = oldHook ? oldHook.queue : []; // 获取fiber上hooks的更新队列
  actions.forEach(action=>{ // 更新state
    if(typeof action === 'function'){// setState执行方法
