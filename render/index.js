...
const TEXT_ELEMENT = "TEXT ELEMENT"; // fiber = {                                      对象结构
...
const isEvent = name => name.startsWith("on"); // 属性是否为事件、子元素、样式


const isAttribute = name => !isEvent(name) && name != "children" && name != "style"; // 属性是否为新属性


const isNew = (prev, next) => key => prev[key] !== next[key]; // 属性是否为旧属性


const isGone = (prev, next) => key => !(key in next); // 更新属性(事件、样式、属性)
...
function resetNextUnitOfWork() {
  ...
  const root = update.from == HOST_ROOT ? update.dom._rootContainerFiber : getRoot(update.instance.__fiber); // 任务从根节点开始
...
function reconcileChildrenArray(wipFiber, newChildElements) {
  ...
  while (index < elements.length || oldFiber != null) {
    ...
    if (element && !sameType) {
      ...
      newFiber = {
        type: element.type,
        tag: typeof element.type === "string" ? HOST_COMPONENT : typeof element.type === 'function' ? FUNCTION_COMPONENT : CLASS_COMPONENT,
        props: element.props,
        parent: wipFiber,
        effectTag: PLACEMENT
      };
...
function useState(initial) {
  ...
  const oldHook = wipFiberRoot.alternate && wipFiberRoot.alternate.hooks && wipFiberRoot.alternate.hooks[hookIndex];
  const hook = {
    state: oldHook ? oldHook.state : initial,
    queue: []
  };
  const actions = oldHook ? oldHook : [];
  ...
  const setState = action => {
    ...
    wipRoot = {
      dom: pendingCommit.dom,
      props: pendingCommit.props,
      alternate: pendingCommit
    };
    nextUnitOfWork = wipRoot;
  };

  wipFiberRoot.hooks.push(hook);
  hookIndex++;
